...
{
    public class Diagnostic_plombController : Controller
    {
        ...
        public ActionResult GetFichiers(int typelogement)
        {
            ...
            try
            {
                ...
                if (typelogement == 1)
                    return PartialView(((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && m.nom_liste == "FICHIER" && m.pc == "O").OrderBy(m => m.libelle).ToList());
                else
                    return PartialView(((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && m.nom_liste == "FICHIER" && m.logt == "O").OrderBy(m => m.libelle).ToList());
        ...
        public ActionResult NewDiag(Guid diag, String cmd)
        {
            ...
            String URL = @"/claimapp/Diagnostic_plomb/Index?id=" + newdiag + "&connect=" + Session["Connect"] + "&cmd=" + cmd;
        ...
        public ActionResult CreateDiag(Guid diag, String cmd)
        {
            ...
            String URL = @"/claimapp/Diagnostic_plomb/Index?id=" + newdiag + "&connect=" + Session["Connect"];
        ...
        public ActionResult SaveDiag(String adresse, String[] liste_logements)
        {
            try
            {
                ...
                List<diag_logement> diags = diag_logement.GetDiag(((Guid)Session["idRapport"])).OrderBy(m => m.gbal).ToList();
                List<RapportLogement> DiagLogements = diag_logement.GetDiagsPlombByIDRapport(((Guid)Session["idRapport"]));

                log.Info("Fichiers : " + Fichiers.Count());
                log.Info("Logements : " + diags.Count());
                log.Info("Rapports : " + DiagLogements.Count());

                bool plomb = ((DiagLogements.Where(m => m.plomb == true).Count() > 0) ? true : false);
                        ...
                        if (connect != null)
                        {
                            ...
                            if (diag != null)
                            {
                                foreach (diag_logement_fichiers f in Fichiers)
                                {
                                    ...
                                    if (String.IsNullOrEmpty(l))
                                        UploadSingleFileToFileCabinet(defaultBasket, "PLOMB", "Immeuble", g, b, a, l, f.nom_fichier, f.type_fichier, CheminFichier, diag.diagnostiqueur, ville, addresse, diag.numcommande, diag.daterapport.Value, diag.correspondant, plomb, Session["Compte"].ToString(), Session["Profil"].ToString());
                                    else
                                        UploadSingleFileToFileCabinet(defaultBasket, "PLOMB", "Logement", g, b, a, l, f.nom_fichier, f.type_fichier, CheminFichier, diag.diagnostiqueur, ville, addresse, diag.numcommande, diag.daterapport.Value, diag.correspondant, plomb, Session["Compte"].ToString(), Session["Profil"].ToString());
                                ...
                                foreach (RapportLogement r in DiagLogements.OrderBy(m => m.gbal))
                                {
                                    ...
                                    XmlElement elementamiante = xmldoc.CreateElement(string.Empty, "Plomb", string.Empty);
                                    XmlText textd_3 = xmldoc.CreateTextNode(r.plomb ? "Oui" : "Non");
                                ...
                                if (!Directory.Exists(System.Configuration.ConfigurationManager.AppSettings["RacineDIAG"] + @"PLOMB\"))
                                    Directory.CreateDirectory(System.Configuration.ConfigurationManager.AppSettings["RacineDIAG"] + @"PLOMB\");

                                xmldoc.Save(System.Configuration.ConfigurationManager.AppSettings["RacineDIAG"] + @"PLOMB\" + ((Guid)Session["idRapport"]) + ".xml");
                            }
        ...
        public static Document UploadSingleFileToFileCabinet(FileCabinet fileCabinet, String Famille, String Patrimoine, String G, String B, String A, String L, string nomdoc, string typedoc, string CheminFichier, string diagnostiqueur, string ville, string addresse, string numcommande, DateTime datediag, string corres, bool plomb, String Compte, String Profil)
        {
            var indexData = new Document()
            {
                Fields = new List<DocumentIndexField>()
                {
                    ...
                    DocumentIndexField.Create("PRESENCE_PLOMB", ((plomb) ? "Oui" : "Non")),
                }
            };
        ...
        public ActionResult DiagEffectues(String[] selection, String[] pieces, bool plomb, bool maj, Guid? idGroupe, string zone, string commentaire)
        {
            try
            {
                ...
                if (maj)
                {
                    ...
                    for (int i = 0; i < selection.Length; i++)
                    {
                        ...
                        {
                            diag_logement.AddDiagPlombEffectues(Guid.Parse(selection[i]), pieces[j], plomb, (Guid)idGroupe, zone, commentaire);
                        }
                    }
                }
                else
                {
                    for (int i = 0; i < selection.Length; i++)
                    {
                        for (int j = 0; j < pieces.Length; j++)
                        {
                            diag_logement.AddDiagPlombEffectues(Guid.Parse(selection[i]), pieces[j], plomb, _idGroupe, zone, commentaire);
                        }
                    }
                }
        ...
        public ActionResult modcontrole(Guid groupe)
        {
            ...
            return Json(new { success = true, logements = _logements, pieces = _pieces, plomb = _plomb, zone = _zone, commentaire = _commentaire }, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var results = diag_logement.DiagsPlomb(((Guid)Session["idRapport"]));
