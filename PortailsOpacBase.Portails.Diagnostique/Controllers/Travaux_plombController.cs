...
{
    public class Travaux_plombController : Controller
    {
        ...
        public ActionResult GetFichiers(int typelogement)
        {
            ...
            try
            {
                WebRequest wb2 = WebRequest.Create(System.Configuration.ConfigurationManager.AppSettings["url_DW_LISTE_DIAG"] + "?limit=400");
                ...
                if (typelogement == 1)
                    return PartialView(((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && m.nom_liste == "FICHIER_TRAVAUX" && m.pc == "O").OrderBy(m => m.libelle).ToList());
                else
                    return PartialView(((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && m.nom_liste == "FICHIER_TRAVAUX" && m.logt == "O").OrderBy(m => m.libelle).ToList());
        ...
        {
            List<ElemDiag> diags = ((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && m.nom_liste == "PIECE_LOGT").OrderBy(m => m.ordre).ToList();
        ...
        {
            List<ElemDiag> diags = ((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && m.nom_liste == "PIECE_PC").OrderBy(m => m.ordre).ToList();
        ...
        {
            List<ElemDiag> diags = ((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && m.nom_liste == "PIECE_DAPP").OrderBy(m => m.ordre).ToList();
        ...
        {
            List<ElemDiag> diags = ((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && m.nom_liste == "PIECE_LOCAUX").OrderBy(m => m.ordre).ToList();
        ...
        {
            List<ElemDiag> diags = ((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && (m.nom_liste == "ZONES")).OrderBy(m => m.ordre).ToList();

            return PartialView(diags);
        }

        public ActionResult listeTravaux()
        {
            List<ElemDiag> diags = ((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "PLOMB" && m.nom_liste == "TRAVAUX").OrderBy(m => m.ordre).ToList();
        ...
        public ActionResult SaveDiag(String adresse, String[] liste_logements)
        {
            try
            {
                ...
                List<diag_logement> diags = diag_logement.GetDiag(((Guid)Session["idRapport"])).OrderBy(m => m.gbal).ToList();
                List<RapportLogement> DiagLogements = diag_logement.GetTravauxByIDRapport(((Guid)Session["idRapport"]));
                        ...
                        if (connect != null)
                        {
                            ...
                            if (diag != null)
                            {
                                foreach (diag_logement_fichiers f in Fichiers)
                                {
                                    ...
                                    if (String.IsNullOrEmpty(l))
                                        UploadSingleFileToFileCabinet(defaultBasket, "PLOMB", "Immeuble", g, b, a, l, f.nom_fichier, f.type_fichier, CheminFichier, diag.diagnostiqueur, ville, addresse, diag.numcommande, diag.daterapport.Value, diag.correspondant, Session["Compte"].ToString(), Session["Profil"].ToString());
                                    else
                                        UploadSingleFileToFileCabinet(defaultBasket, "PLOMB", "Logement", g, b, a, l, f.nom_fichier, f.type_fichier, CheminFichier, diag.diagnostiqueur, ville, addresse, diag.numcommande, diag.daterapport.Value, diag.correspondant, Session["Compte"].ToString(), Session["Profil"].ToString());
                                ...
                                foreach (RapportLogement r in DiagLogements.OrderBy(m => m.gbal))
                                {
                                    ...
                                    XmlElement elementpiece = xmldoc.CreateElement(string.Empty, "Piece", string.Empty);
                                    XmlText textd_2 = xmldoc.CreateTextNode(r.piece);
                                    elementpiece.AppendChild(textd_2);
                                    newdiag.AppendChild(elementpiece);

                                    XmlElement elementzone = xmldoc.CreateElement(string.Empty, "Commentaire", string.Empty);
                                    XmlText textd_5 = xmldoc.CreateTextNode(((r.commentaire != null) ? r.commentaire : ""));
                                    elementzone.AppendChild(textd_5);
                                    newdiag.AppendChild(elementzone);

                                    XmlElement elementamiante = xmldoc.CreateElement(string.Empty, "ElementTraite", string.Empty);
                                    XmlText textd_3 = xmldoc.CreateTextNode(r.element_traite);
                                    elementamiante.AppendChild(textd_3);
                                    newdiag.AppendChild(elementamiante);

                                    XmlElement elementprelev = xmldoc.CreateElement(string.Empty, "NatureTravaux", string.Empty);
                                    XmlText textd_4 = xmldoc.CreateTextNode(r.nature_travaux);
                                ...
                                if (!Directory.Exists(System.Configuration.ConfigurationManager.AppSettings["RacineDIAG"] + @"TRAVAUX_PLOMB\"))
                                    Directory.CreateDirectory(System.Configuration.ConfigurationManager.AppSettings["RacineDIAG"] + @"TRAVAUX_PLOMB\");

                                xmldoc.Save(System.Configuration.ConfigurationManager.AppSettings["RacineDIAG"] + @"TRAVAUX_PLOMB\" + ((Guid)Session["idRapport"]) + ".xml");
                            }
        ...
        public ActionResult TravauxEffectues(String[] selection, String[] pieces, string element_traite, string nature_travaux, string commentaire, bool maj, Guid? idGroupe)
        {
            try
            {
                ...
                if (maj)
                {
                    ...
                    for (int i = 0; i < selection.Length; i++)
                    {
                        ...
                        {
                            diag_logement.AddTravauxEffectues(Guid.Parse(selection[i]), pieces[j], (Guid)idGroupe, element_traite, nature_travaux, commentaire);
                        }
                    }
                }
                else
                {
                    for (int i = 0; i < selection.Length; i++)
                    {
                        for (int j = 0; j < pieces.Length; j++)
                        {
                            diag_logement.AddTravauxEffectues(Guid.Parse(selection[i]), pieces[j], _idGroupe, element_traite, nature_travaux, commentaire);
                        }
                    }
                }
        ...
        public ActionResult DelDiagByID(Guid id)
        {
            bool result = diag_logement.DelDiagLigneByID(id);
        ...
        public ActionResult modcontrole(Guid groupe)
        {
            ...
            String _elements_traites = String.Empty;
            String _nature_travaux = String.Empty;
            ...
            foreach (diag_logement_pieces p in lignes)
            {
                ...
                _pieces.Add(p.piece);
                _zone = p.zone;
                _elements_traites = p.elements_traites;
                _nature_travaux = p.nature_travaux;
                _commentaire = p.commentaires;
            }

            return Json(new { success = true, logements = _logements, pieces = _pieces, elements_traites = _elements_traites, nature_travaux = _nature_travaux, zone = _zone, commentaire = _commentaire }, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var results = diag_logement.Travaux(((Guid)Session["idRapport"]));
        ...
        public ActionResult Valider(String g, String b, String a, String cmd, String societe, String corres, String nomdiag, String daterapport, String datedepot, int controle)
        {
            ...
            try
            {
                ...
                if (((List<Logement>)Session["Adresses"]).Count > 0)
                {
                    ...
                    {
                        Session["a"] = false;
                        Session["LogementEnCours"] = ((List<Logement>)Session["Adresses"]).First(m => String.IsNullOrEmpty(m.numloc));

                        return Json(new { trouve = true, adresse = ((List<Logement>)Session["Adresses"]).First(m => String.IsNullOrEmpty(m.numloc)).nomcom }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                        Session["a"] = true;
                        ...
                        return Json(new { trouve = true, adresse = ((List<Logement>)Session["Adresses"]).First(m => String.IsNullOrEmpty(m.numloc)).adresse + " - " + ((List<Logement>)Session["Adresses"]).First(m => String.IsNullOrEmpty(m.numloc)).nomcom }, JsonRequestBehavior.AllowGet);
                    }
                }
                else
