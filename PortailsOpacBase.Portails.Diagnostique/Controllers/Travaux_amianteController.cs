...
{
    public class Travaux_amianteController : Controller
    {
        ...
        public ActionResult GetFichiers(int typelogement)
        {
            ...
            try
            {
                ...
                log.Info("Types fichiers trouvés : " + ((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "AMIANTE" && m.nom_liste == "FICHIER_TRAVAUX").Count());

                return PartialView(((ListeDiag)Session["ListeDiag"]).items.Where(m => (m.type_diag == "AMIANTE" || m.type_diag == "AMIANTE\n") && m.nom_liste == "FICHIER_TRAVAUX").OrderBy(m => m.libelle).ToList());
                    ...
                    try
                    {
                        if (connect != null)
                        {
                            ...
                            {  
                                ...
                                if (!Directory.Exists(System.Configuration.ConfigurationManager.AppSettings["RacineDIAG"] + @"TRAVAUX_AMIANTE\"))
                                    Directory.CreateDirectory(System.Configuration.ConfigurationManager.AppSettings["RacineDIAG"] + @"TRAVAUX_AMIANTE\");

                                xmldoc.Save(System.Configuration.ConfigurationManager.AppSettings["RacineDIAG"] + @"TRAVAUX_AMIANTE\" + ((Guid)Session["idRapport"]) + ".xml");
                            }
        ...
        {
            List<ElemDiag> diags = ((ListeDiag)Session["ListeDiag"]).items.Where(m => m.type_diag == "AMIANTE" && m.nom_liste == "TRAVAUX").OrderBy(m => m.ordre).ToList();

            log.Info("Liste travaux trouvée : " + diags.Count);
